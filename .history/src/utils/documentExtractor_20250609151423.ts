...
export const extractTextFromPowerPoint = async (file: File): Promise<string> => {
  try {
    ...
    if (extractedText.length < 50) {
      ...
      // If still insufficient, provide a meaningful fallback
      if (extractedText.length < 50) {
        throw new Error('PowerPoint file does not contain sufficient readable text content for quiz generation. Please ensure your presentation has text-based content on the slides.');
      }
    }
