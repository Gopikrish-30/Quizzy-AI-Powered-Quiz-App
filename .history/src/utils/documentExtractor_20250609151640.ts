...
export const extractTextFromPowerPoint = async (file: File): Promise<string> => {
  try {
    ...
    // Debug logging
    console.log('Raw extracted text sample:', extractedText.substring(0, 200));
    console.log('Extracted text length:', extractedText.length);

    // If we didn't get enough meaningful text, try alternative extraction
    if (extractedText.length < 50) {
      console.warn('Limited text extracted from PowerPoint file, trying alternative method');

      // Try a more aggressive text extraction approach
      ...
      // If still insufficient, provide a helpful message about the limitation
      if (extractedText.length < 50) {
        throw new Error(`PowerPoint file contains limited extractable text content. This often happens when:

• Text is embedded in images (screenshots, diagrams, charts)
• Content is primarily visual (pictures, graphics)
• Text is part of complex layouts or shapes

For better quiz generation, try:
• Using text boxes instead of image text
• Converting images with text to actual PowerPoint text elements
• Providing a document with more text-based content

Current extractable content: "${extractedText.substring(0, 100)}${extractedText.length > 100 ? '...' : ''}"`);
      }
    }
