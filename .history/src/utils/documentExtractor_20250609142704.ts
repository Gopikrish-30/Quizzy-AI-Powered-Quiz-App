...
export const extractTextFromPowerPoint = async (file: File): Promise<string> => {
  try {
    ...
    // This is a simplified approach - in a production app, you might want to use a more robust library
    const arrayBuffer = await file.arrayBuffer();
    
    // Convert to text using a simple approach
    // Note: This is a basic implementation and might not work for all PPT files
    const text = await extractTextFromBinaryFile(arrayBuffer, file.name);
    
    console.log('PowerPoint text extracted, length:', text.length);
    return text;
  } catch (error) {
