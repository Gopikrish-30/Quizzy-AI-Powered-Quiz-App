...
export const extractTextFromPowerPoint = async (file: File): Promise<string> => {
  console.log('PowerPoint text extraction for file:', file.name);

  // For now, provide a user-friendly message about PowerPoint limitations
  // and suggest alternatives since PowerPoint text extraction is complex
  const message = `
PowerPoint files often contain text embedded in images, charts, and complex layouts that are difficult to extract automatically.

To generate a quiz from your PowerPoint content, you have a few options:

1. **Convert to text**: Copy the text content from your slides and paste it into the topic field instead
2. **Save as PDF**: Export your PowerPoint as a PDF file, which often preserves text better
3. **Use Word format**: Copy your content into a Word document (.docx) for better text extraction

Would you like to try one of these alternatives? The text-based content will generate much better quiz questions.

If you'd like to proceed anyway, please note that the quiz may be based on limited extracted content.
  `.trim();

  // Try basic extraction anyway
  try {
    const arrayBuffer = await file.arrayBuffer();
    let extractedText = '';
    ...
    // If we got some meaningful text, use it
    if (extractedText.length > 100) {
      return extractedText;
    }

    // Otherwise, provide the helpful message
    return message;
